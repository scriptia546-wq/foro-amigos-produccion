...
from sqlalchemy import Boolean, DateTime, ForeignKey, Integer, String, Text, UniqueConstraint, and_, create_engine, inspect, or_, text
...
app = FastAPI(title="Círculo Privado API", version="5.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class Post(Base):
    ...
    file_url: Mapped[Optional[str]] = mapped_column(String(700), nullable=True)
...
class SavedPost(Base):
    ...
    __table_args__ = (UniqueConstraint("post_id", "user_id", name="unique_saved_post"),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    post_id: Mapped[int] = mapped_column(ForeignKey("posts.id"), nullable=False)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), nullable=False)
    active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
...
class PollVote(Base):
    ...
    __table_args__ = (UniqueConstraint("poll_id", "option_id", "user_id", name="unique_vote_per_option"),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    poll_id: Mapped[int] = mapped_column(ForeignKey("polls.id"), nullable=False)
...
class Friendship(Base):
    ...
    __table_args__ = (UniqueConstraint("requester_id", "receiver_id", name="unique_friend_request"),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
...
class PollVoteRequest(BaseModel):
    option_ids: list[int] = Field(min_length=1, max_length=8)
...
def normalize_channel(channel: str) -> str:
    ...
    if c == "coordinacion-salidas":
        c = "coordinación-salidas"
...
def parse_datetime(value: Optional[str]) -> Optional[datetime]:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Fecha inválida. Usa el selector de fecha y hora.") from exc
...
class ConnectionManager:
    ...
    async def connect(self, user_id: int, ws: WebSocket) -> None:
        ...
        await self.broadcast({"type": "presence_updated", "user_id": user_id, "online": True})
...
def is_online(user_id: int) -> bool:
    return user_id in manager.active


def users_are_friends(db: Session, a: int, b: int) -> bool:
    return db.query(Friendship).filter(
        Friendship.status == "accepted",
        or_(
            and_(Friendship.requester_id == a, Friendship.receiver_id == b),
            and_(Friendship.requester_id == b, Friendship.receiver_id == a),
        ),
    ).first() is not None


def serialize_user(user: Optional[User], current_user_id: Optional[int] = None, db: Optional[Session] = None) -> Optional[dict]:
    if not user:
        return None
    status = None
    friendship_id = None
    if current_user_id and db and user.id != current_user_id:
        fr = db.query(Friendship).filter(
            or_(
                and_(Friendship.requester_id == current_user_id, Friendship.receiver_id == user.id),
                and_(Friendship.requester_id == user.id, Friendship.receiver_id == current_user_id),
            )
        ).first()
        if fr:
            status = fr.status
...
def serialize_poll(poll: Optional[Poll], current_user_id: int, db: Session) -> Optional[dict]:
    if not poll:
        return None
    selected = [
        v.option_id
        for v in db.query(PollVote).filter(PollVote.poll_id == poll.id, PollVote.user_id == current_user_id).all()
    ]
...
def serialize_event(event: Optional[Event]) -> Optional[dict]:
    if not event:
        return None
    return {
        "id": event.id,
...
def serialize_post(post: Post, current_user_id: int, db: Session) -> dict:
    ...
    saved = db.query(SavedPost).filter(SavedPost.post_id == post.id, SavedPost.user_id == current_user_id, SavedPost.active == True).first() is not None  # noqa: E712
    return {
        "id": post.id,
        ...
        "author": serialize_user(post.author, current_user_id, db),
        "poll": serialize_poll(post.poll, current_user_id, db),
        "event": serialize_event(post.event),
    }
...
def serialize_dm(dm: DirectMessage, current_user_id: int, db: Session) -> dict:
    sender = db.query(User).filter(User.id == dm.sender_id).first()
    ...
    return {
        ...
        "receiver": serialize_user(receiver),
        "mine": dm.sender_id == current_user_id,
    }


async def notify_user(db: Session, user_id: int, actor_id: Optional[int], type_: str, message: str, entity_id: Optional[int] = None) -> None:
    n = Notification(user_id=user_id, actor_id=actor_id, type=type_, message=message, entity_id=entity_id)
    db.add(n)
    db.commit()
    db.refresh(n)
...
def ensure_schema_upgrades() -> None:
    # Permite subir esta versión sobre la base SQLite/PostgreSQL anterior sin borrar datos.
    ensure_column("users", "birth_date", "VARCHAR(20)")
    ensure_column("posts", "file_url", "VARCHAR(700)")
    ensure_column("posts", "file_name", "VARCHAR(260)")
    ensure_column("posts", "file_type", "VARCHAR(80)")
    ensure_column("direct_messages", "message_type", "VARCHAR(20) DEFAULT 'text'")
    ensure_column("direct_messages", "file_url", "VARCHAR(700)")
    ensure_column("direct_messages", "file_name", "VARCHAR(260)")
...
def startup() -> None:
    ...
    try:
        if db.query(User).count() == 0:
            for username, display in [("rodrigo", "Rodrigo"), ("mateo", "Mateo"), ("valeria", "Valeria"), ("lucas", "Lucas")]:
                db.add(
                    User(
                        username=username,
                        ...
                        token=create_token(),
                        avatar_initials=build_initials(display),
                    )
                )
            db.commit()
    finally:
...
def register(p: RegisterRequest, db: Session = Depends(get_db)):
    username = p.username.strip().lower().replace(" ", "")
    display = (p.display_name or p.username).strip()
    if not username:
    ...
    user = User(
        username=username,
        display_name=display,
        birth_date=(p.birth_date or "").strip() or None,
...
def update_me(p: ProfileUpdateRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if p.display_name is not None and p.display_name.strip():
        user.display_name = p.display_name.strip()
        user.avatar_initials = build_initials(user.display_name)
    if p.bio is not None:
        user.bio = p.bio.strip()
    if p.avatar_url is not None:
        user.avatar_url = p.avatar_url.strip() or None
    db.commit()
    db.refresh(user)
    return serialize_user(user, user.id, db)
...
async def upload(file: UploadFile = File(...), user: User = Depends(get_current_user)):
    ...
    if not cloud_name or not api_key or not api_secret:
        raise HTTPException(status_code=500, detail="Cloudinary no está configurado en Render. Llena CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY y CLOUDINARY_API_SECRET.")
    if cloud_name.lower() in {"root", "cloudinary", "api keys"}:
    ...
    if len(content) > max_bytes:
        raise HTTPException(status_code=413, detail="El archivo es demasiado pesado.")
    try:
        result = cloudinary.uploader.upload(
            content,
            folder=f"circulo_privado/user_{user.id}",
            resource_type="auto",
            overwrite=False,
            use_filename=True,
        ...
        raise HTTPException(status_code=502, detail=f"No se pudo subir el archivo a Cloudinary: {exc}") from exc
    return {
...
async def friend_request(receiver_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not fr:
        fr = Friendship(requester_id=user.id, receiver_id=receiver_id, status="pending")
        db.add(fr)
    elif fr.status == "rejected":
        fr.status = "pending"
        fr.requester_id = user.id
        fr.receiver_id = receiver_id
        fr.updated_at = datetime.utcnow()
    elif fr.status == "accepted":
        return {"ok": True, "friendship": {"id": fr.id, "status": fr.status}}
    db.commit()
    db.refresh(fr)
    await notify_user(db, receiver_id, user.id, "friend_request", f"{user.display_name} quiere agregarte como amigo.", fr.id)
    return {"ok": True, "friendship": {"id": fr.id, "status": fr.status}}
...
async def respond_friend(friendship_id: int, action: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if fr.receiver_id != user.id:
        raise HTTPException(status_code=403, detail="Solo quien recibe responde.")
    ...
    await manager.broadcast({"type": "friends_updated"})
    return {"ok": True}


@app.get("/friends")
def friends(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    rels = db.query(Friendship).filter(
        Friendship.status == "accepted",
    ...
    for r in rels:
        ...
        if u:
            out.append(serialize_user(u, user.id, db))
    return {"online": [u for u in out if u["online"]], "offline": [u for u in out if not u["online"]]}
...
def posts(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ps = db.query(Post).options(
        selectinload(Post.author),
        selectinload(Post.liked_by),
        selectinload(Post.poll).selectinload(Poll.options),
...
async def create_post(p: PostCreateRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    post = Post(
        ...
        channel=normalize_channel(p.channel),
    ...
    res = serialize_post(post, user.id, db)
    await manager.broadcast({"type": "new_post", "post": res})
    return res
...
async def delete_post(post_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if poll:
        db.query(PollVote).filter(PollVote.poll_id == poll.id).delete(synchronize_session=False)
...
async def like(post_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not post:
        raise HTTPException(status_code=404, detail="Post no encontrado.")
    like_row = db.query(PostLike).filter(PostLike.post_id == post_id, PostLike.user_id == user.id).first()
    if like_row and like_row.active:
        like_row.active = False
        post.likes = max(0, post.likes - 1)
        liked = False
    elif like_row:
        like_row.active = True
        post.likes += 1
        liked = True
    else:
        db.add(PostLike(post_id=post_id, user_id=user.id, active=True))
        post.likes += 1
        liked = True
    db.commit()
    await manager.broadcast({"type": "like_updated", "post_id": post.id, "likes": post.likes})
    return {"post_id": post.id, "likes": post.likes, "liked_by_me": liked}


@app.post("/posts/{post_id}/save")
def save_post(post_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    post = db.query(Post).filter(Post.id == post_id).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post no encontrado.")
    saved = db.query(SavedPost).filter(SavedPost.post_id == post_id, SavedPost.user_id == user.id).first()
    if saved and saved.active:
        ...
        db.add(SavedPost(post_id=post_id, user_id=user.id, active=True))
        is_saved = True
    db.commit()
    return {"post_id": post_id, "saved_by_me": is_saved}
...
async def add_comment(post_id: int, p: CommentCreateRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not post:
        raise HTTPException(status_code=404, detail="Post no encontrado.")
    c = Comment(content=p.content.strip(), post_id=post.id, author_id=user.id)
    post.comments_count += 1
    db.add(c)
    db.commit()
    db.refresh(c)
    c = db.query(Comment).options(selectinload(Comment.author)).filter(Comment.id == c.id).first()
    res = serialize_comment(c)
    await manager.broadcast({"type": "new_comment", "post_id": post.id, "comment": res, "comments_count": post.comments_count})
    return res
...
async def create_poll(p: PollCreateRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    post = Post(content=p.question.strip(), channel=normalize_channel(p.channel), post_type="poll", author_id=user.id)
    db.add(post)
    db.commit()
    db.refresh(post)
    poll = Poll(question=p.question.strip(), multiple=p.multiple, post_id=post.id)
    db.add(poll)
    db.commit()
    db.refresh(poll)
    for t in opts[:8]:
        db.add(PollOption(text=t, poll_id=poll.id))
    db.commit()
...
async def vote(poll_id: int, p: PollVoteRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for old in db.query(PollVote).filter(PollVote.poll_id == poll.id, PollVote.user_id == user.id).all():
        ...
        db.delete(old)
    for oid in ids:
        opt = db.query(PollOption).filter(PollOption.id == oid).first()
        if opt:
            opt.votes += 1
    ...
    res = serialize_post(post, user.id, db)
    await manager.broadcast({"type": "poll_updated", "post": res})
    return res


@app.post("/events")
async def create_event(p: EventCreateRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    post = Post(content=p.title.strip(), channel=normalize_channel(p.channel), post_type="event", author_id=user.id)
    db.add(post)
    db.commit()
    db.refresh(post)
    ev = Event(
        ...
        start_at=parse_datetime(p.start_at),
        location=(p.location or "").strip() or None,
        post_id=post.id,
    )
    db.add(ev)
    db.commit()
...
async def update_event(event_id: int, p: EventUpdateRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if post.author_id != user.id:
        raise HTTPException(status_code=403, detail="Solo el autor modifica.")
    if p.title is not None and p.title.strip():
        ev.title = p.title.strip()
        post.content = ev.title
    if p.description is not None:
        ev.description = p.description.strip() or None
    if p.start_at is not None:
        ev.start_at = parse_datetime(p.start_at)
    if p.location is not None:
        ev.location = p.location.strip() or None
    db.commit()
    post = db.query(Post).options(selectinload(Post.author), selectinload(Post.liked_by), selectinload(Post.event)).filter(Post.id == post.id).first()
    res = serialize_post(post, user.id, db)
    await manager.broadcast({"type": "post_updated", "post": res})
    return res


@app.get("/dm/{friend_id}")
def get_dm(friend_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if not users_are_friends(db, user.id, friend_id):
    ...
    ).order_by(DirectMessage.created_at.asc()).limit(160).all()
...
async def send_dm(friend_id: int, p: DirectMessageCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if not users_are_friends(db, user.id, friend_id):
        raise HTTPException(status_code=403, detail="Solo puedes chatear con amigos aceptados.")
    msg_type = p.message_type if p.message_type in {"text", "image", "file", "link", "poll"} else "text"
    dm = DirectMessage(
        sender_id=user.id,
    ...
    db.commit()
    db.refresh(dm)
    sender = serialize_dm(dm, user.id, db)
    receiver = serialize_dm(dm, friend_id, db)
    await manager.send_to_user(friend_id, {"type": "direct_message", "message": receiver})
    await manager.send_to_user(user.id, {"type": "direct_message", "message": sender})
    return sender
...
async def websocket_endpoint(websocket: WebSocket, token: str):
    ...
    try:
        ...
        if user_id:
            ...
            await manager.broadcast({"type": "presence_updated", "user_id": user_id, "online": False})
        db.close()
...
def health():
    return {
        "status": "ok",
        "message": "Círculo Privado V3 API funcionando.",
